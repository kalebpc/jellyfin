...
If ($Recurse) {
    ...
    ForEach ( $folderfile In $folders ) {
        If ($folderfile.PSIsContainer) {
            ...
            If ( $($temp | Split-Path -Leaf) -iin $libraryFolderNames ) {
                # Example 1 is in libraryfoldernames so traverse path one less parent
                ...
                # Out : 1.G:\Jellyfin\Shows
                # Out : 2. G:\Jellyfin\Shows\Show Name (TV Series 2005-2013) [imdbid-tt1234567]
                If ( $($temp | Split-Path -Leaf) -in $libraryFolderNames ) {
                    # Need the title not a folder name. Out : 1.G:\Jellyfin\Shows
                    $temp = $folderfile.FullName | Split-Path -Parent | Split-Path -Parent | Split-Path -Leaf
